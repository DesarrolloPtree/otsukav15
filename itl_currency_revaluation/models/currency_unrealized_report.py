...
class ShellAccount(object):
    ...
    def get_lines(self):
        ...
        sql = """SELECT res_partner.name,
                   ...
                   account_move.state as move_status,
                   (CASE WHEN res_partner.name IS NOT NULL AND account_move.name IS NOT NULL THEN res_partner.name || ' - ' || account_move.name
                       WHEN res_partner.name IS NULL AND account_move.name IS NOT NULL THEN account_move.name
                       WHEN res_partner.name IS NOT NULL AND account_move.name IS NULL THEN res_partner.name
                       ELSE '--'
                    END) as l_name
        ...
        #_logger.info("=====> ordered_lines: " + str(result))
        self.ordered_lines = result
...
class CurrencyUnrealizedReport(models.AbstractModel):
    _inherit = "report.account_multicurrency_revaluation.curr_unrealized_report"
