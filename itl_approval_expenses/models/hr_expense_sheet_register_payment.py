...
class HrExpenseSheetRegisterPaymentWizard(models.TransientModel):
    ...
    def expense_post_payment(self):
        ...
        # ITL: relate payment to expense
        expense_sheet.itl_payment_ids = [(4, payment.id)]
        # ITL: mark expense as paid if expense sheet has not journal entry
        if not expense_sheet.account_move_id:
        ...
        ### Invoice payment
        account_move_lines_to_reconcile = self.env['account.move.line']
        for line in payment.move_line_ids + expense_sheet.expense_line_ids.mapped('invoice_related_id').line_ids:
            if line.account_id.internal_type == 'payable' and not line.reconciled:
                account_move_lines_to_reconcile |= line
        account_move_lines_to_reconcile.reconcile()
