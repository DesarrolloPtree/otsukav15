...
from odoo.addons.hr_expense.models.hr_expense import HrExpenseSheet
...
def action_sheet_move_create(self):
    ...
    # ITL: Adding lines without invoice leated
    expense_line_ids = self.mapped('expense_line_ids')\
        .filtered(lambda r: not float_is_zero(r.total_amount, precision_rounding=(r.currency_id or self.env.company.currency_id).rounding) and not r.invoice_related_id)
    res = expense_line_ids.action_move_create()

    if not self.accounting_date:
        self.accounting_date = self.account_move_id.date

    # ITL: Removing conditions
    ...
    # ITL: Posting invoice related
    for expense in self.expense_line_ids:
        if expense.invoice_related_id:
            if expense.invoice_related_id.state == 'draft':
                if expense.invoice_related_id.itl_force_post:
                    lock_date = max(self.company_id.period_lock_date or date.min, self.company_id.fiscalyear_lock_date or date.min)
                    if self.user_has_groups('account.group_account_manager'):
                        lock_date = self.company_id.fiscalyear_lock_date
                    expense.invoice_related_id.date = lock_date + timedelta(days=1)
...
HrExpenseSheet.action_sheet_move_create = action_sheet_move_create
...
class HrExpenseCustom(models.Model):
    ...
    def action_move_create(self):
        ...
        for expense in self:
            ...
            if expense.payment_mode == 'company_account' and expense.company_id.itl_automatic_payment:
    ...
    def _prepare_move_values(self):
        move_values = super(HrExpenseCustom, self)._prepare_move_values()

        if self.sheet_id.itl_force_post:
            lock_date = max(self.sheet_id.company_id.period_lock_date or date.min, self.sheet_id.company_id.fiscalyear_lock_date or date.min)
            if self.user_has_groups('account.group_account_manager'):
                lock_date = self.sheet_id.company_id.fiscalyear_lock_date
            move_values['date'] = lock_date + timedelta(days=1)
        _logger.info("move_values['date']: " + str(move_values['date']))
        return move_values
...
class HrExpenseSheetCustom(models.Model):
    ...
    itl_force_post = fields.Boolean(string="Force post", compute="_compute_tax_lock_date_message")
    ...
    def open_clear_advance(self):
        vals = super(HrExpenseSheetCustom, self).open_clear_advance()
    ...
    def _compute_tax_lock_date_message(self):
        for sheet in self:
            sheet.itl_force_post = False
            lock_date = max(sheet.company_id.period_lock_date or date.min, sheet.company_id.fiscalyear_lock_date or date.min)
            if self.user_has_groups('account.group_account_manager'):
                lock_date = sheet.company_id.fiscalyear_lock_date
            if sheet.state == 'approve':
                for line in sheet.expense_line_ids:
                    if line.date <= (lock_date or date.min):
                    #if line.company_id.tax_lock_date and line.date and line.date <= line.company_id.tax_lock_date:
                        sheet.itl_force_post = True