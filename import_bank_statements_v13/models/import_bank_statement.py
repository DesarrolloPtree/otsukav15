...
class import_bank_statements(models.TransientModel):
    ...
    import_option = fields.Selection([('csv', 'CSV'),('xls', 'XLS'),('xml', 'XML'),('txt','TXT')],string='Tipo', readonly=True)
    ...
    def onchnage_uploaded_file(self):
        if self.file:
            ...
            if file_ext.lower() not in ('csv','xls','xlsx','xml','txt'):
                raise ValidationError('Solo se permiten archivo con extensión csv, xls, xlsx, xml o txt.')
    ...
    def process_file(self):
        ...
        if self.journal_id.import_bank_statement_method == 'banamex':
            lines = self.banamex_txt()
    ...
    def banamex_txt(self):
        lines = []
        if self.import_option == 'txt':
            txt_data = self.get_txt_data()
            lines = self.get_values_from_txt_data_banamex(txt_data)
            non_duplicate_list, duplicate_list = self.validate_duplicate_lines(lines)
            self.line_ids = non_duplicate_list
            self.duplicate_line_ids = duplicate_list
    
    def get_txt_data(self):
        txt_data = base64.b64decode(self.file)
        txt_data = txt_data.decode('latin-1')
        return txt_data
    
    def get_values_from_txt_data_banamex(self, txt_data):
        lines = []
        abs = self.env['account.bank.statement']
        balance_start = abs._get_opening_balance(self.journal_id.id)
        cum_bal = balance_start
        
        file_lines = txt_data.split('\n')
        number_of_lines = len(file_lines)
        start = False
        i = 1
        lines = []
        for row in file_lines:
            #_logger.info("====> type: " + str(type(row)) + " row: " + str(row))
            val = {}
            
            if i == 2:
                i += 1
                continue
            if number_of_lines == i:
                break
            if start:
                fecha = row[0:11].strip()
                
                if not self.is_date(fecha):
                    break
                datetimeobject = datetime.strptime(fecha,'%d/%m/%Y')
                hora = row[11:19].strip()
                importe = row[19:39].strip().replace(',','')
                divisa = row[39:43].strip()
                ref_num = row[43:56].strip()
                ref_alf_num = row[56:77].strip()
                clave_rast = row[77:98].strip()
                concepto_transact = row[98:139].strip()
                tipo_operacion = row[139:153].strip()
                nombre_ordenante = row[153:274].strip()
                cuenta_ordenante = row[274:293].strip()
                rfc_odenante = row[293:307].strip()
                banco_odenante = row[307:428].strip()
                cuenta_virtual = row[428:445].strip()
                num_operacion = row[445:len(row)].strip()

                ref = ''
                if ref_alf_num != '' and ref_num != '':
                    ref = ref_alf_num + "|" + ref_num
                if ref_alf_num != '' and ref_num == '':
                    ref = ref_alf_num
                if ref_alf_num == '' and ref_num != '':
                    ref = ref_num
                ...
                partner_id = False
                if nombre_ordenante != '':
                    if 'JUAN LUIS GARDU' in nombre_ordenante:
                        nombre_ordenante = 'JUAN LUIS GARDU'
                    partner_id = self.env['res.partner'].search([('name','ilike',nombre_ordenante)])
                    if partner_id:
                        partner_id = partner_id[0].id
                
                val.update({
                            'name': concepto_transact,
                            'date': datetimeobject,
                            'ref': ref,
                ...
                lines.append((0, 0, val))
                i += 1
            if 'DETALLE DE ACTIVIDAD' in row:
                i += 1
                start = True
        return lines
