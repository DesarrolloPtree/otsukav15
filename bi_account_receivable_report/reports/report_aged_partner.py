...
class BiReportAgedPartnerBalance(models.AbstractModel):
    _name = 'report.bi_account_receivable_report.bi_report_rec'

    def  get_partner_name(self, partner_ids):
         partner_all_name  = ''
         for partner_id  in   partner_ids:
             partner_name = self.env['res.partner'].browse(partner_id)
             partner_all_name +=  partner_name.name  + ',' 
         return partner_all_name
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length,partner_ids):
        ...
        if len(partner_ids) > 0:
           arg_list += (date_from, tuple(company_ids),tuple(partner_ids))
        else:
        ...
        for line in self.env['account.move.line'].browse(aml_ids):
            ...
            for partial_line in line.matched_debit_ids:
                ...
                dt = date_from_obj.date()
                if partial_line.max_date <= dt:
    ...
    def _get_report_values(self, docids, data=None):
        ...
        movelines, total, dummy = self._get_partner_move_lines(account_type, date_from, target_move,
                                                               data['form']['period_length'],partner_ids)
        partner_names = self.get_partner_name(partner_ids)
        return {
            'doc_ids': self.ids,
            'doc_model': model,
            'data': data['form'],
            ...
            'get_direction': total,
            'get_partner_names': partner_names,
